...
func (c *PrometheusCollector) collectContainersInfo(ch chan<- prometheus.Metric) {
	...
	for _, cont := range containers {
		...
		desc := prometheus.NewDesc("container_restart_count", "Number of times the container has been restarted", labels, nil)
		ch <- prometheus.MustNewConstMetric(desc, prometheus.GaugeValue, float64(restartcnt), values...)
